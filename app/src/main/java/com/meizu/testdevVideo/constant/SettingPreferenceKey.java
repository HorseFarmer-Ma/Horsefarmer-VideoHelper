package com.meizu.testdevVideo.constant;

/**
 * Created by maxueming on 2016/11/3.
 */
public class SettingPreferenceKey {
    public static String LOCK_WIFI = "preference_lock_wifi";
    public static String KEEP_WAKEUP = "preference_wakeup";
    public static String MUTE = "preference_mute";
    public static String MUTE_RUN_TASK = "preference_mute_run_task";
    public static String MONKEY_FLOAT_BTN = "preference_monkey_float_btn";
    public static String LOCK_WIFI_TYPE = "preference_lock_wifi_type";
    public static String DEFINED_WIFI_SSID = "preference_defined_wifi_ssid";
    public static String DEFINED_WIFI_PSW = "preference_defined_wifi_psw";
    public static String SINGLE_LOG_SIZE = "preference_single_log_size";
    public static String ALL_LOG_SIZE = "preference_all_log_size";
    public static String CLEAR_CACHE = "preference_clear_cache";
    public static String MONKEY_MTK_SET = "preference_monkey_mtk_set";
    public static String CLEAR_LOG = "preference_clear_log";
    public static String CATCH_LOG_TYPE = "preference_catch_log_type";

    public static String APP_TYPE = "preference_app_type";
    public static String MONKEY_CHOOSE_APP = "monkey_choose_app";
    public static String MONKEY_REPORT_SEND_FAIL_REASON = "monkey_report_send_fail_reason";
    public static String MONKEY_DEFINED_CHOOSE_APP = "monkey_defined_choose_app";
    public static String EMAIL_ADDRESS = "email_address";
    public static String MONKEY_PACKAGE = "monkey_package";
    public static String IS_MONKEY_REPORT_SEND_SUCCESS = "is_monkey_report_send_success";
    public static final String CLEAR_CACHE_ORIGN_TIME= "10";

    // 上次清除缓存时间
    public static String LSAT_CLEAR_CACHE_TIME = "lsat_clear_cache_time";

    public static String APP_PACKAGE_NAME = "app_package_name";
}
